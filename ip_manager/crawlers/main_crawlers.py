...
class BaseCrawler:
    ...
    def crawl(self, ips, many=False):
        if many:
            ...
            if undone_ips or undone_ips is None:
                undone_ips = self.normal_crawl(ips[:], many)
            return undone_ips
    ...
    def api_crawl(self, ips, many=False):
        if many:
            ...
            if undone_ips:
                return self._api_crawling_switch(ips[:], many, use_proxy=False)
    ...
    def normal_crawl(self, ips, many=False):
        web = self.driver_obj.get_driver(use_proxy=True)
        if many:
            undone_ips = self._normal_crawling_switch(web, ips[:], many)
            if undone_ips:
                web = self.driver_obj.get_driver(use_proxy=False)
                return self._normal_crawling_switch(web, ips[:], many)
            return undone_ips

        instance = self._normal_crawling_switch(web, ips, many)
        if instance is None:
            web = self.driver_obj.get_driver(use_proxy=False)
            instance = self._normal_crawling_switch(web, ips, many)
        return instance

    def _normal_crawling_switch(self, web, ips, many=False):
        raise NotImplementedError

    def _api_crawling_switch(self, ips, many=False, use_proxy=False):
        raise NotImplementedError

    def _login_crawl(self, web, ips, many):
        raise NotImplementedError

    def _no_login_crawl(self, web, ips, many):
        raise NotImplementedError
...
class IPInfoCrawler(BaseCrawler):
    ...
    def _normal_crawling_switch(self, web, ips, many=False):
        if many:
            undone_ips = self._no_login_crawl(web, ips, many)
            if undone_ips:
                undone_ips = self._login_crawl(web, undone_ips, many)
            return undone_ips

        instance = self._no_login_crawl(web, ips, many)
        if instance is None:
            instance = self._login_crawl(web, ips, many)
        return instance

    def _api_crawling_switch(self, ips, many=False, use_proxy=False):
        return

    def _login_crawl(self, web, ips, many=False):
        try:
            with web as driver:
    ...
    def _no_login_crawl(self, web, ips, many=False):
        try:
            with web as driver:
                wait = WebDriverWait(driver, timeout=settings.DRIVER_TIMEOUT)
            driver.get(self.url)
        ...
        try:
            with web as driver:
                ...
                for ip in ips:
                    ...
                    ip_input_box = driver.find_element(By.XPATH, self.ip_input_path_n_login2)
                    submit_button = driver.find_element(By.XPATH, self.submit_butt_path_n_login_2)
                    sleep(2)
                    ip_input_box.clear()
                    ips.remove(ip)
                    sleep(3)
...
class RipeCrawler(BaseCrawler):
    ...
    def _normal_crawling_switch(self, web, ips, many=False):
        if many:
            undone_ips = self._no_login_crawl(web, ips, many)
            if undone_ips:
                undone_ips = self._login_crawl(web, ips, many)
            return undone_ips
